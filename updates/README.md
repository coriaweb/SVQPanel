...
| 0121 | El correo saliente salía por IPv6 (Postfix default `smtp_address_preference=any`). La IPv6 del servidor casi nunca tiene PTR y su SPF no cubre la IPv6 que el SO elige del /64 → Gmail marcaba `spf=fail ... does not designate <IPv6>`. La IPv4 dedicada sí tiene PTR+SPF. Se fija `smtp_address_preference=ipv4`; un dominio activa IPv6 dedicada (opt-in con PTR) desde el panel. Solo si hay Postfix | 2026-07-11 |
