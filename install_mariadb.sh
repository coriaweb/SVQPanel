...
if [[ ! -f "$PMA_TMP" || ! -s "$PMA_TMP" ]]; then
    ...
    # ── Usuario interno pma (controluser) + BD phpmyadmin ────────────────────
...
PMADBEOF
    ...
    echo -e "  ✓ BD phpmyadmin y usuario pma creados"

    # ── config.inc.php — signon auth + configuration storage ─────────────────
 ...
 * Acceso directo sin token → redirigido al panel.
 */
...
PMACFGEOF
    ...
    echo -e "  ✓ config.inc.php creado (signon + configuration storage)"
