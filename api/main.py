...
async def startup():
    create_tables()
    _run_migrations()
    # Backups programados: hilo de fondo DENTRO del proceso del panel (los imports
    # pesados de backup_scheduler están a nivel de módulo → se cargan una vez, sin
    # la fuga de memoria que tenía la versión antigua que re-importaba en cada
    # tick). Sustituye al timer systemd que arrancaba un proceso Python entero
    # cada minuto (ruido en el log + ~1s CPU x1440/día para 0 jobs casi siempre).
    try:
...
def _run_migrations():
    ...
    migrations = [
        ...
        "ALTER TABLE settings ADD COLUMN IF NOT EXISTS pwd_min_length INTEGER DEFAULT 12",
        "ALTER TABLE settings ADD COLUMN IF NOT EXISTS pwd_require_upper BOOLEAN DEFAULT TRUE",
        "ALTER TABLE settings ADD COLUMN IF NOT EXISTS pwd_require_lower BOOLEAN DEFAULT TRUE",
        "ALTER TABLE settings ADD COLUMN IF NOT EXISTS pwd_require_digit BOOLEAN DEFAULT TRUE",
        "ALTER TABLE settings ADD COLUMN IF NOT EXISTS pwd_require_symbol BOOLEAN DEFAULT FALSE",
        ...
        # Paralelismo de las copias: cuántos dominios a la vez. NULL/0 = automático
        # según CPU, RAM y carga (scripts/worker_pool.py). Es ajuste del SERVIDOR
        # (Ajustes → Archivos), no de cada job: no tiene sentido por cliente.
        "ALTER TABLE settings ADD COLUMN IF NOT EXISTS backup_max_workers INTEGER",
        ...
        "ALTER TABLE backup_jobs ADD COLUMN IF NOT EXISTS s3_bucket     VARCHAR(255)",
        ...
        # Cache del análisis de ataque WP (lo refresca un cron cada ~3h)
        "ALTER TABLE domains ADD COLUMN IF NOT EXISTS wp_xmlrpc_hits INTEGER NOT NULL DEFAULT 0",
        "ALTER TABLE domains ADD COLUMN IF NOT EXISTS wp_wplogin_hits INTEGER NOT NULL DEFAULT 0",
        "ALTER TABLE domains ADD COLUMN IF NOT EXISTS wp_attack_checked_at TIMESTAMP",
        # ─────────────────────────────────────────────────────────────────
...
app.include_router(migration_sources.router, prefix="/api", tags=["Migration Sources"])
